...
'''This program is a Simple Convolutional Neural Network Program that trains the model over Dataset MNIST.
The function CRP_layer is the functions added to vary the number of convolutional layers and pooling layers in the model.
import tweak as T in the above program calls the Tweak.py programs and T.run envokes the run function of tweak module .
T.run goes to tweak.py and gets the arguements inputs for the funtion CRP_layer . These arguments are - 
a) no. of CRP layers
b) no. of filters per layer
c) size of each filter
d) size of each pool
e) size of each stride 
By default , only 1 layer is added to the model and when the model is trained first time,it is trained with 1 CRP layer only .

Now , the issue comes to accuracy , the new model is only saved if the accuracy of the model is greater than the previous accuracy . 
And for that , the accuracy of each model is compared with the accuracy of previous model stored in a file accuracy.txt . By default ,the file contains value 0. '''
